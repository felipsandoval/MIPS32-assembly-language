...
## atoi-4.asm -- ASCII to integer recursive mode
## code made by Felipe Sandoval Sibada

## Arquitectura de Computadores curso 2017-2018 
## Computer's Architecture (2017-2018)
## atoi_1.asm -- converts ASCII to iNTEGERS Version3
## code made by Felipe Sandoval Sibada

.data
myArray: .space 1024
instruction: .asciiz "Insert your sentence: "
...
main:
	...
	syscall
	add $t0, $zero, $zero 			# My index position to go through the array
...
loop:
	...
	bgt $s0, 127, finish			# Checking if is an ASCII Char
	blt  $s0, 0, finish              # Checking if is an ASCII Char
	add $t1, $t1, $s0				
	add $t0, $t0, 1
	lb $s0, myArray($t0)
	jal loop
